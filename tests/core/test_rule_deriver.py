...
class TestSingleTagFilter:
    ...
    def test_role_arn_preserved(self):
        config = _make_config(
            _make_rule("rule-1", {"Tag": {"Key": "k", "Value": "v"}})
        )
        result = derive_rules(_SRC_BUCKET, config)
        assert result[0].replication_role_arn == _ROLE_ARN
...
class TestProperty1RuleDerivation:
    ...
    @given(
        ...
        role_arn=st.just("arn:aws:iam::111122223333:role/my-role"),
        dest_arn=st.just("arn:aws:s3:::my-dest-bucket"),
    )
    @settings(max_examples=100)
    def test_tag_filter_fields_preserved_exactly(
        self, tag_pairs: dict, role_arn: str, dest_arn: str
    ) -> None:
        """Tag key-value pairs, role ARN, and destination ARN are preserved (Req 3.2).
        ...
        assert rule.replication_role_arn == role_arn
