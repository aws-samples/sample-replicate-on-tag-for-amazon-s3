...
Task 15.2 — mocked integration tests for create/submit:
  - CreateJob called with manifest and Batch Operations job role, no destination parameter.
...
_ROLE_ARN = "arn:aws:iam::123456789012:role/s3rot-batch-operations-role"
...
def _call_submit(
    ...
    return submit_batch_job(
        ...
        batch_operations_role_arn=role_arn,
        config_id=config_id,
...
class TestCreateJobIntegration:
    ...
    def test_create_job_uses_batch_operations_role_arn(self):
        """RoleArn in CreateJob is the stack-created job role (Req 7.2)."""
        client = _mock_s3control()
        _call_submit(client=client, role_arn=_ROLE_ARN)
        kwargs = client.create_job.call_args[1]
        assert kwargs["RoleArn"] == _ROLE_ARN

    def test_create_job_role_arn_is_the_configured_value_verbatim(self):
        """Whatever ARN the caller supplies is the one passed (Req 2.1, 2.5)."""
        other = "arn:aws:iam::123456789012:role/some-other-batchops-role"
        client = _mock_s3control()
        _call_submit(client=client, role_arn=other)
        assert client.create_job.call_args[1]["RoleArn"] == other

    def test_audit_entry_records_the_batch_operations_role_arn(self):
        """The batch_job_created audit entry names the role passed (Req 2.6)."""
        client = _mock_s3control()
        emitted: list[dict] = []
        with patch(
            "src.adapters.batch_operations_adapter.observability.emit",
            side_effect=emitted.append,
        ):
            _call_submit(client=client, role_arn=_ROLE_ARN)

        audits = [e for e in emitted if e.get("action") == "batch_job_created"]
        assert len(audits) == 1
        # log_audit merges details flat into the entry.
        assert audits[0]["batch_operations_role_arn"] == _ROLE_ARN
        assert "replication_role_arn" not in audits[0]
...
class TestConditionallyEnabledCompletionReport:
    def test_feature_on_report_enabled_with_all_tasks_scope(self):
        ...
        submit_batch_job(
            ...
            batch_operations_role_arn=_ROLE_ARN,
    ...
    def test_report_always_enabled_with_defaults(self):
        """Using defaults (no explicit prefix/state_bucket overrides) still
        produces Report.Enabled=True — the report is unconditional
        (Requirement 1.1)."""
        client = _mock_s3control()
        _call_submit(client=client)
        kwargs = client.create_job.call_args[1]
    ...
    def test_report_enabled_with_topic_arn_unset(self):
        """Report.Enabled is True even when completion tracking is not
        configured — the report is always requested (Requirement 1.1)."""
        client = _mock_s3control()
        submit_batch_job(
            s3control_client=client,
            account_id=_ACCOUNT_ID,
