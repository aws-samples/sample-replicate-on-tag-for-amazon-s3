...
Environment variables (see deploy/README.md for the full reference):
  ...
  S3ROT_TEST_ROLE_ARN: IAM role ARN for replication (the bucket's own
    replication configuration role, which performs delivery)
  S3ROT_TEST_BATCHOPS_ROLE_ARN: the stack's BatchOperationsRoleArn output --
    the Batch Operations job role the Solution creates and passes as each
    job's RoleArn
...
Optional:
  S3ROT_TEST_KMS_KEY_ARN: symmetric CMK ARN. When set, the state object and the
    ...
    never reaches, which are the branches where a defect can sit unnoticed
    because nothing else executes them. Requires the
    key policy from deploy/README.md "KMS Key Setup", which must name the
    BatchOperationsRoleArn output alongside ExecutionRoleArn, or the batch job
    cannot read the manifest.
    See docs/kms.md "Verifying a KMS-enabled deployment" for the checks
...
BATCHOPS_ROLE_ARN = _get_env_or_skip(
    "S3ROT_TEST_BATCHOPS_ROLE_ARN",
    "the stack BatchOperationsRoleArn output",
)
...
def query_journal(athena, since_timestamp=None):
    ...
    # storage_class is projected but never filtered on in SQL, matching
    # athena_journal_adapter: the Archived_Object_Filter has to run after
    # deduplication, so excluding archived rows here would defeat it.
    query = (
        "SELECT bucket, key, version_id, "
        "CAST(object_tags AS JSON) AS resulting_tags, "
        "sequence_number, record_timestamp AS event_time, storage_class "
    ...
    while True:
        ...
        for row in rows[start:]:
            ...
            # Column order: bucket, key, version_id, resulting_tags,
            # sequence_number, event_time, storage_class
            ...
            # Optional in the journal, so absent is not evidence of archival.
            storage_class   = values[6] if len(values) > 6 and values[6] else None
...
def assert_sse_kms(s3_client, bucket: str, key: str, label: str) -> None:
    ...
    the encryption downgrade this check exists to catch, where a handler
    rewrites an SSE-KMS state object under SSE-S3, and a warning in a long
    console log is easy to miss.
    """
...
def main() -> int:
    ...
    # -----------------------------------------------------------------------
    # [Archived]  Drop objects S3 will not replicate because they are archived
    #
    # Positioned here, after deduplication and before matching, to mirror
    # orchestrator._select_eligible. The ordering matters: a lifecycle
    # transition into an archived class shares a logical_operation_id with the
    # tagging record it follows, so dedup keeps the transition (higher
    # sequence_number) and this filter then sees the archived class. Applied
    # any earlier, the pre-transition record would survive and reach a
    # manifest. candidate_hwm is deliberately left as computed over all
    # eligible operations so the watermark advances past an archived object
    # rather than re-reading it forever.
    # -----------------------------------------------------------------------
    print("[Archived] Applying Archived_Object_Filter...")
    deduped, archived_ops = filter_archived_operations(deduped)
    if archived_ops:
        print(
            f"[Archived] {len(archived_ops)} excluded as archived "
            f"({count_by_storage_class(archived_ops)}); "
            f"{len(deduped)} kept"
        )
    else:
        print(f"[Archived] 0 excluded, {len(deduped)} kept")

    if not deduped:
        print("[Archived] All operations excluded as archived — no job to submit")
        return 2
    ...
    # tag-scoped rule's matches. The job runs as the stack-created Batch
    # Operations role, not as the bucket's replication role.
    #
    ...
    print(
        ...
        f"bucket={SOURCE_BUCKET!r}  role={BATCHOPS_ROLE_ARN!r}..."
    )
    try:
        ...
        # catch a defect in the request the Solution builds, so a malformed
        # Manifest.Location.ManifestEncryption would sit in exactly that blind
        # spot and this script would not see it.
        # The completion report is always enabled — every job writes one.
        # The report is the only place per-task failure reasons exist; without
        # it a total failure is undiagnosable after the fact.
        report_prefix = _completion_report_prefix(
            SOURCE_BUCKET, written.s3_location.key
        )
        # The job role reads the manifest and writes the completion report
        # under grants in its own identity policy, provisioned by the stack.
        # Nothing is primed on the State Bucket's policy: a run touches no
        # bucket policy at all.
        #
        # The specific claim under test is that the role passed is the
        # stack-created job role and not the bucket's replication role, so
        # assert it rather than infer it from the job succeeding.
        assert BATCHOPS_ROLE_ARN != ROLE_ARN, (
            "S3ROT_TEST_BATCHOPS_ROLE_ARN must be the stack's own job role, "
            "not the bucket's replication role"
        )
        ...
        submission = submit_batch_job(
            ...
            batch_operations_role_arn=BATCHOPS_ROLE_ARN,
            config_id=SOURCE_BUCKET,
