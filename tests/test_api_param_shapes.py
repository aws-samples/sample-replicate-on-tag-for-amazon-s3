...
class TestOtherS3CallSites:
    ...
    def test_bucket_policy_write_is_valid(self):
        client = MagicMock()
        ...
        bucket_policy_adapter.ensure_completion_report_bucket_policy(
            client, _STATE_BUCKET, _SRC_BUCKET, _ROLE_ARN, _ACCOUNT
        )
        ops = assert_calls_match_api(client, "s3")
        assert "PutBucketPolicy" in ops, (
            "the policy write must be exercised, not just the read"
        )
