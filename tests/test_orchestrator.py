...
class TestEndToEndSingleInterval:
    ...
    def test_failed_submission_marks_the_bucket_errored(self):
        ...
        job that was never created has no record and no status. The KMS-only
        submission defect fixed in 0.1.13 therefore ran unnoticed with every
        metric reading healthy.
        """
    ...
    def test_replication_role_passed_to_submit(self):
        """The existing replication role ARN is passed to the batch job (Req 7.2)."""
        _, _, mock_submit, _ = _run_with_mocks(["my-bucket"])
        kwargs = mock_submit.call_args.kwargs
        assert kwargs["replication_role_arn"] == _ROLE_ARN
...
def _run_completion_tracking_hook(
    ...
    mock_read_bops_report = MagicMock(
        side_effect=read_bops_report_side_effect,
        return_value=[] if read_bops_report_side_effect is None else None,
    )
    ...
    with (
        ...
        patch(
            "src.orchestrator.bucket_policy_adapter.ensure_completion_report_bucket_policy",
            return_value=False,
        ),
    ):
        run_interval(_config([bucket_name]), rt)
...
class TestCompletionRecordCreationHook:
    ...
    def test_exception_in_read_bops_report_is_caught_and_logged(self):
        """An exception raised during the BOPS report read or
        store.merge_completion_configs is caught, logged, and does not
        affect the existing circuit-breaker logic (Requirement 6.1)."""
        ...
        assert len(errors) == 1
        assert "job-boom" in errors[0]["cause"]
    ...
    def test_disabled_when_topic_arn_absent(self):
        """When completion_report_topic_arn is unset, neither call happens
        at all (feature no-op, Requirement 4.8's orchestrator-side gate)."""
    ...
    def test_disabled_when_topic_arn_absent_for_any_status(self, status):
        """When completion_report_topic_arn is unset, neither call happens
        regardless of job status (Requirement 4.8's orchestrator-side
        gate) — complements test_disabled_when_topic_arn_absent (Complete)."""
    ...
    def test_skip_when_job_exists_other_logic_continues_for_failed_status(self):
        """When completion_job_exists is True, both calls are skipped,
        but the existing Failed circuit-breaker readmit logic for that job
        still runs normally (Requirement 2.4)."""
...
# ---------------------------------------------------------------------------
# Task 26.5/26.6/26.7: Pre-submission bucket-policy priming call site
# (design.md Decision 10, Requirements 9.1, 9.2, 9.7)
# ---------------------------------------------------------------------------

_BP_TOPIC_ARN = _CT_TOPIC_ARN


def _run_bucket_policy_priming(
    completion_report_topic_arn: str | None = _BP_TOPIC_ARN,
    ensure_policy_side_effect=None,
    bucket_name: str = "my-bucket",
    ops_per_bucket: dict[str, list[TaggingOperation]] | None = None,
):
    """Run one interval exercising only the pre-submission bucket-policy
    priming call site (task 26.5) — no prior submissions, so the
    DescribeJob-loop creation hook (task 17.1) never fires and the only
    completion-tracking-gated calls are record_scan_result and the new
    ensure_completion_report_bucket_policy priming call.

    Returns (emitted_events, mock_store, mock_ensure_policy, mock_submit_job).
    """
    mock_factory_cls = MagicMock()
    mock_factory = MagicMock()
    mock_factory_cls.return_value = mock_factory
    ...
    mock_ensure_policy = MagicMock(side_effect=ensure_policy_side_effect)
    ...
    return emitted, mock_store, mock_ensure_policy, mock_submit_job


class TestBucketPolicyPrimingUnitTests:
    """Task 26.7: call-ordering and feature-flag-gating unit tests for the
    pre-submission bucket-policy priming call site (task 26.5).

    Requirements: 9.1, 9.2
    """

    def test_priming_call_precedes_submit_batch_job_for_same_config(self):
        """The priming call, when it runs, executes before submit_batch_job
        for the same config_id (Requirement 9.1)."""
        call_order: list[str] = []
        ...
        def ensure_policy_ordered(*args, **kwargs):
            call_order.append("ensure_completion_report_bucket_policy")
            return False

        def submit_ordered(*args, **kwargs):
            call_order.append("submit_batch_job")
            return _submitted()
        ...
        with (
            ...
            patch(
                "src.orchestrator.batch_operations_adapter.submit_batch_job",
                side_effect=submit_ordered,
            ),
            patch("src.orchestrator.preflight_count", return_value=0),
            patch("src.orchestrator.read_permanent_deletes", return_value=set()),
            patch(
                "src.orchestrator.bucket_policy_adapter.ensure_completion_report_bucket_policy",
                side_effect=ensure_policy_ordered,
            ),
        ):
            run_interval(_config(["my-bucket"]), {**_BASE_RUNTIME, "completion_report_topic_arn": _BP_TOPIC_ARN})
    ...
    def test_no_bucket_policy_call_when_topic_arn_absent(self):
        """When completion_report_topic_arn is not configured, no
        GetBucketPolicy/PutBucketPolicy call occurs — the adapter call
        itself is never reached (Requirement 9.2)."""
        emitted, mock_store, mock_ensure_policy, mock_submit_job = (
            _run_bucket_policy_priming(completion_report_topic_arn=None)
        )
        mock_ensure_policy.assert_not_called()
        # submit_batch_job still happens (the rest of submission is unaffected).
        assert mock_submit_job.call_count == 1

    def test_no_priming_call_for_config_with_nothing_to_submit(self):
        """A config_id with nothing to submit this run (no matching journal
        ops → no manifest → `continue` before the submission point) never
        triggers a bucket_policy_adapter call."""
        emitted, mock_store, mock_ensure_policy, mock_submit_job = (
            _run_bucket_policy_priming(ops_per_bucket={"my-bucket": []})
        )
        mock_ensure_policy.assert_not_called()
        mock_submit_job.assert_not_called()

    def test_priming_call_receives_expected_arguments(self):
        """The priming call is passed the state_bucket, the bucket-name
        sentinel (task 4.1 collapses the per-config loop into one per-bucket
        path), and the bucket's single replication_role_arn (design.md D2)."""
        emitted, mock_store, mock_ensure_policy, mock_submit_job = (
            _run_bucket_policy_priming()
        )
        mock_ensure_policy.assert_called_once()
        call_args = mock_ensure_policy.call_args
        # positional args: (s3_client, state_bucket, config_id, replication_role_arn)
        assert call_args.args[1] == _STATE_BUCKET
        assert call_args.args[2] == "my-bucket"
        assert call_args.args[3] == _ROLE_ARN

    def test_priming_failure_logged_and_submission_still_happens(self):
        """A priming failure is logged via observability.log_error naming
        the affected bucket, and submit_batch_job still runs unconditionally
        (Requirement 9.7)."""
        emitted, mock_store, mock_ensure_policy, mock_submit_job = (
            _run_bucket_policy_priming(
                ensure_policy_side_effect=RuntimeError("policy boom")
            )
        )
        errors = [
            e for e in emitted
        ...
        assert len(errors) == 1
        assert "my-bucket" in errors[0]["cause"]
        assert mock_submit_job.call_count == 1


# ---------------------------------------------------------------------------
# Task 26.6: Property 22 — a bucket-policy priming failure never prevents
# or alters the subsequent submission outcome (Requirement 9.7)
# Feature: source-status-completion-tracking, Property 22: A bucket-policy priming failure never prevents or alters the subsequent submission outcome
# ---------------------------------------------------------------------------


class TestProperty22BucketPolicyPrimingFailureIsolation:
    """# Feature: source-status-completion-tracking, Property 22: A bucket-policy priming failure never prevents or alters the subsequent submission outcome
    ...
    @given(
        raise_policy_error=st.booleans(),
    )
    @settings(max_examples=100)
    def test_injected_priming_failure_does_not_change_submission_outcome(
        self, raise_policy_error: bool
    ) -> None:
        """# Feature: source-status-completion-tracking, Property 22: A bucket-policy priming failure never prevents or alters the subsequent submission outcome"""
        side_effect = RuntimeError("injected priming failure") if raise_policy_error else None

        # Baseline: no injection.
        _, baseline_store, _, baseline_submit = _run_bucket_policy_priming(
            ensure_policy_side_effect=None
        )
        # Injected run — potentially failing, per the hypothesis-drawn flag.
        _, injected_store, _, injected_submit = _run_bucket_policy_priming(
            ensure_policy_side_effect=side_effect
        )

        # submit_batch_job is still called, with identical arguments — every
        # argument except the per-run client-mock instance (a fresh
        # MagicMock is constructed by ClientFactory on each independent
        # _run_bucket_policy_priming call and is never compared for
        # equality by the real implementation) is byte-for-byte identical.
        assert baseline_submit.call_count == injected_submit.call_count == 1
        baseline_call_kwargs = dict(baseline_submit.call_args.kwargs)
        injected_call_kwargs = dict(injected_submit.call_args.kwargs)
        baseline_call_kwargs.pop("s3control_client", None)
        injected_call_kwargs.pop("s3control_client", None)
        assert baseline_call_kwargs == injected_call_kwargs

        # The resulting SubmissionRecord/checkpoint/lease state is identical.
        baseline_store.record_submission.assert_called_once()
        injected_store.record_submission.assert_called_once()
        baseline_rec: SubmissionRecord = baseline_store.record_submission.call_args[0][2]
        injected_rec: SubmissionRecord = injected_store.record_submission.call_args[0][2]
        assert baseline_rec.status == injected_rec.status
        assert baseline_rec.consecutive_failures == injected_rec.consecutive_failures
        assert baseline_rec.job_id == injected_rec.job_id

        baseline_release_kwargs = baseline_store.release_lease.call_args.kwargs
        injected_release_kwargs = injected_store.release_lease.call_args.kwargs
        assert (
            baseline_release_kwargs.get("submitted_refs") is None
        ) == (
            injected_release_kwargs.get("submitted_refs") is None
        )
...
# ---------------------------------------------------------------------------
# Replication role ARN is validated before any use
#
# AWS Security Agent finding f-e6fa06f0-42ab-4927-93cb-1f3a88cd0ce4: the role
# ARN was validated only on the bucket-policy path, and that path runs only
# when completion reporting is enabled. With completion reporting off, an
# unvalidated ARN read from the bucket's replication configuration reached
# CreateJob as the job role. Validation now happens where the ARN is derived,
# so it covers both consumers.
# ---------------------------------------------------------------------------


class TestReplicationRoleArnValidatedBeforeUse:
    _FOREIGN = "arn:aws:iam::999988887777:role/attacker-role"
    _MALFORMED = "not-an-arn"
    _WILDCARD = "*"

    def _run_with_role_arn(self, role_arn):
        """Run one interval whose sole rule carries *role_arn*.

        Completion reporting is left off (no completion_report_topic_arn in the
        runtime config), which is the configuration in which the ARN previously
        reached CreateJob unchecked.
        """
        bucket = "my-bucket"
    ...
    @pytest.mark.parametrize("bad", [_FOREIGN, _MALFORMED, _WILDCARD])
    def test_bad_arn_never_reaches_create_job(self, bad):
        _, mock_submit, _ = self._run_with_role_arn(bad)
        assert mock_submit.call_count == 0, (
            "an unvalidated replication role ARN reached submit_batch_job"
        )

    @pytest.mark.parametrize("bad", [_FOREIGN, _MALFORMED, _WILDCARD])
    def test_bad_arn_reported_as_an_error(self, bad):
        emitted, _, _ = self._run_with_role_arn(bad)
        causes = " ".join(e.get("cause", "") for e in _errors(emitted))
        assert "not a well-formed IAM role ARN" in causes

    def test_same_account_arn_still_submits(self):
        """The guard must not block the normal case."""
        good = f"arn:aws:iam::{_ACCOUNT_ID}:role/rep-role"
        _, mock_submit, _ = self._run_with_role_arn(good)
        assert mock_submit.call_count == 1
