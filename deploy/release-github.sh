...
#   4. Builds package.zip, generates package.zip.sha256.
#   5. Creates a GitHub Release with template.yaml, package.zip, and package.zip.sha256.
#
# Prerequisites:
...
mkdir -p "$TMP_DIR"
"$GITLAB_ROOT/deploy/build-package.sh" --build-only "$TMP_DIR/package.zip"
shasum -a 256 "$TMP_DIR/package.zip" | awk '{print $1 "  package.zip"}' > "$TMP_DIR/package.zip.sha256"
...
echo "Assets: template.yaml, package.zip, package.zip.sha256"
