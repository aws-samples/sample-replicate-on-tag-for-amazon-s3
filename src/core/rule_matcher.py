...
def match(
    ...
    * Multiple rules belonging to the same ``replication_config_id`` are
      ...
      ``destination_bucket_arns`` is the corresponding union of those rules'
      destination bucket ARNs, carried for reporting only. A rule whose
      configuration records no destination bucket contributes nothing, so the
      set can be empty.
    * Deterministic: identical inputs produce identical outputs regardless
    ...
    # Destination bucket ARNs of the satisfied rules, accumulated per config_id
    # alongside the rule IDs so a Completion_Report can name where the object
    # was bound for. A config_id spanning several rules contributes each rule's
    # destination, so this is a set rather than a single value.
    destinations_by_config: dict[str, set[str]] = {}
    for rule in bucket_rules:
        if _rule_satisfies(op.resulting_tag_set, op.object_key, rule):
            ...
            destination_arn = rule.destination.bucket_arn
            if destination_arn:
                destinations_by_config.setdefault(
                    rule.replication_config_id, set()
                ).add(destination_arn)
    ...
    matched: set[MatchedObject] = {
        MatchedObject(
            ...
            destination_bucket_arns=frozenset(
                destinations_by_config.get(config_id, set())
            ),
        )
