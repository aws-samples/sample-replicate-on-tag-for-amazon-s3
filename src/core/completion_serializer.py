...
def _serialize_tracked_object(obj: TrackedObject) -> dict[str, Any]:
    ...
    # Sorted lists, not sets: JSON has no set type, and a stable order keeps
    # the serialized state object byte-comparable across writes. Omitted when
    # empty so a state object written before these fields existed round-trips
    # unchanged.
    if obj.matched_rules:
        result["matched_rules"] = sorted(obj.matched_rules)
    if obj.destinations:
        result["destinations"] = sorted(obj.destinations)
    return result
...
def _deserialize_tracked_object(item_key: str, data: dict[str, Any]) -> TrackedObject:
    ...
    return TrackedObject(
        ...
        matched_rules=frozenset(data.get("matched_rules") or ()),
        destinations=frozenset(data.get("destinations") or ()),
    )
