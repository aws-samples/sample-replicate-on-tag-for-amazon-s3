...
def submit_batch_job(
    ...
    completion_report_prefix: str | None = None,
    ...
    source bucket's **existing** Replication_Configuration and the supplied
    replication IAM role.  No customer-supplied destination is required or
    accepted (7.2).
    ...
    replication_role_arn:
        ARN of the **existing** replication IAM role attached to the bucket's
    ...
    completion_report_prefix:
        The BOPS_Completion_Report prefix (design.md Decision 4), or
        ``None``. When set (``CompletionNotificationEmail`` is configured
        for the deployment), the job is created with an enabled
        BOPS_Completion_Report (``Report.Enabled=True``,
        ``Report.ReportScope="AllTasks"``, ``Report.Format=
        "Report_CSV_20180820"``) written to this prefix in ``state_bucket``.
        When ``None`` (the feature is not configured), the job is created
        with ``Report={"Enabled": False}`` — the exact pre-this-feature
        behavior, unchanged (Requirement 1.1, 1.4).
    state_bucket:
        The State_Bucket name the BOPS_Completion_Report is written to.
        Required (and used to build the ``Report.Bucket`` ARN) only when
        ``completion_report_prefix`` is set; ignored otherwise.
    ...
    if is_inventory:
        ...
        # the job's RoleArn, which is why deploy/README.md's KMS Key Setup
        # tells operators to grant the replication role kms:Decrypt on the key.
    else:
    ...
    try:
        ...
        create_response = _call_with_timeout(
            lambda: s3control_client.create_job(
                ...
                Priority=10,
                # 7.2 — pass the existing replication IAM role (iam:PassRole)
    ...
    # Audit: record the iam:PassRole action — which replication role was passed
    # to which job for which config/bucket. This is the privilege-relevant fact
    # a security investigation needs to detect anomalous role usage; create_job
...
def _build_report_spec(
    completion_report_prefix: str | None, state_bucket: str | None
) -> dict:
    """Build the CreateJob ``Report`` field (design.md Decision 4).

    When ``completion_report_prefix`` is set (feature enabled), returns an
    enabled BOPS_Completion_Report spec written to ``state_bucket`` under
    that prefix, with ``ReportScope="AllTasks"`` so every processed object
    ...
    When ``completion_report_prefix`` is ``None`` (feature disabled),
    returns ``{"Enabled": False}`` — the exact pre-this-feature behavior,
    unchanged (Requirement 1.4).

    Requirements: 1.1, 1.2, 1.3, 1.4
    """
    if completion_report_prefix is None:
