...
dict without interpreting them, so a payload whose keys are not the per-bucket
sentinel yields no entry for that bucket and
:meth:`get_submission_records` returns an empty dict rather than failing.
:meth:`record_submission` always writes the single bucket-keyed entry,
...
class StateStore:
    ...
    * ``PutObject`` with ``If-None-Match: *`` to create the object the first
    ...
    def get_submission_records(
        ...
        a bucket has a single ``SubmissionRecord``. Returns an empty dict when
        no submission records are stored, when the payload holds no
        sentinel-keyed entry, or when the state object does not yet exist.
    ...
    def merge_completion_configs(
        ...
        # Look up pre-stored timestamps and routing from the state payload
        # (written at manifest-generation time by store_completion_timestamps).
    ...
    def mark_report_diagnosed(
        ...
        """Set ``report_diagnosed = True`` on the submission record for *config_id*.

        Follows the same read-raw-JSON, mutate-one-key, single-attempt
        conditional-write pattern as :meth:`increment_submission_failure_streak`.

        The caller (``StateWriter``) wraps this in a best-effort handler:
        a failed write costs one duplicate diagnostic log on the next run,
        ...
        Args:
            ...
            state_bucket:  The scratch/state bucket name.
            source_bucket: The Monitored_Bucket whose state object to update.
            config_id:     The key under ``submission_records`` to update
                           (the per-bucket sentinel, equal to source_bucket).
            current_etag:  The ETag from the caller's most recent observation.
        ...
        if config_id in records:
            records[config_id]["report_diagnosed"] = True
